 ...
 */
fun <T> styledSpinner(vararg styleClass: String): Spinner<T> = styledSpinner(styleClass =
        *styleClass) { }

/**
 * Add a styled [Spinner] to this manager.
 */
fun <T> NodeManager.styledSpinner(vararg styleClass: String): Spinner<T> = styledSpinner(styleClass
        = *styleClass) { }

/**
 * Create a styled [Spinner] with configuration block.
 */
inline fun <T> styledSpinner(vararg styleClass: String, configuration: (@LayoutDslMarker
        Spinner<T>).() -> Unit): Spinner<T> {
    contract { callsInPlace(configuration, EXACTLY_ONCE) }
    return newChild(Spinner<T>(), styleClass = *styleClass, configuration = configuration)
}

/**
 * Add a styled [Spinner] with configuration block to this manager.
 */
inline fun <T> NodeManager.styledSpinner(vararg styleClass: String, configuration: (@LayoutDslMarker
        Spinner<T>).() -> Unit): Spinner<T> {
    contract { callsInPlace(configuration, EXACTLY_ONCE) }
    return addChild(newChild(Spinner<T>(), styleClass = *styleClass, configuration = configuration))
}
