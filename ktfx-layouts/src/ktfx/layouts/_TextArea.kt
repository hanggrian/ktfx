 ...
 */
fun styledTextArea(text: String? = null, vararg styleClass: String): TextArea = styledTextArea(text =
        text, styleClass = *styleClass) { }

/**
 * Add a styled [TextArea] to this manager.
 */
fun NodeManager.styledTextArea(text: String? = null, vararg styleClass: String): TextArea =
 ...
 */
fun NodeManager.textArea(text: String? = null): TextArea = textArea(text = text) { }

/**
 * Create a [TextArea] with configuration block.
 */
inline fun textArea(text: String? = null, configuration: (@LayoutDslMarker TextArea).() -> Unit):
        TextArea {
    contract { callsInPlace(configuration, EXACTLY_ONCE) }
