...
): PrefixSelectionComboBox<T> = ktfx.layouts.controlsfx.prefixSelectionComboBox(init)()
...
inline fun <T> LayoutManager<Node>.styledPrefixSelectionComboBox(
    styleClass: String,
    noinline init: ((@LayoutDsl PrefixSelectionComboBox<T>).() -> Unit)? = null
): PrefixSelectionComboBox<T> = ktfx.layouts.controlsfx.styledPrefixSelectionComboBox(styleClass, init)()