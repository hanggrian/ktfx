...
): ContextMenu = javafxx.layouts.contextMenu(*items, init = init).also { setContextMenu(it) }
...
inline fun Control.styledContextMenu(
    ...
    vararg items: MenuItem,
    noinline init: ((@LayoutDsl _ContextMenu).() -> Unit)? = null
): ContextMenu = javafxx.layouts.styledContextMenu(styleClass, *items, init = init).also { setContextMenu(it) }
