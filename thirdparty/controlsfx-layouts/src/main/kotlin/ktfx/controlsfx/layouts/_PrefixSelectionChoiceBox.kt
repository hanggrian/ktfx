 ...
 * @return the control added.
 */
public inline fun <T> NodeManager.prefixSelectionChoiceBox(configuration: (@KtfxLayoutDslMarker
        PrefixSelectionChoiceBox<T>).() -> Unit): PrefixSelectionChoiceBox<T> {
    contract { callsInPlace(configuration, EXACTLY_ONCE) }
    val child = PrefixSelectionChoiceBox<T>()
    child.configuration()
 ...
 */
public fun <T> NodeManager.styledPrefixSelectionChoiceBox(vararg styleClass: String, id: String? =
        null): PrefixSelectionChoiceBox<T> = styledPrefixSelectionChoiceBox(styleClass =
        *styleClass, id = id) { }
